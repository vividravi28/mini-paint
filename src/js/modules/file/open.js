...
class File_open_class {
  ...
  events() {
    ...
    window.addEventListener("message", function (event) {
      ...
      if (messageData.type === "LOAD_IMAGE") {
        ...
        console.log("Image URL:", messageData.data?.substring(0, 50) + "...");
        console.log("URL length:", messageData.data?.length || 0);
        ...
        if (messageData.data) {
          ...
          if (messageData.data.startsWith("data:")) {
            ...
            messageData.data.startsWith("http://") ||
            messageData.data.startsWith("https://")
          ) {
            // HTTP URL - use URL handler
            console.log("Loading HTTP URL");
            _this.file_open_url_handler({ url: messageData.data });
          } else {
            console.error(
              "❌ Unknown URL format:",
              messageData.data.substring(0, 50)
            );
          }
        } else {
  ...
  maybe_file_open_url_handler() {
    ...
    // Support both legacy "image" param and alternative "imageUrl"
    const resource = url_params.image || url_params.imageUrl;
